...
const AdminDashboard = () => {
  ...
  const fetchStats = async () => {
    try {
      const response = await axios.get('/api/events/admin/stats');
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/events/admin/registrations');
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'events':
        return <ManageEvents events={events} loading={loading} onNavigate={setActiveTab} />;
      case 'create':
        return <CreateEvent />;
      case 'registrations':
        return <StudentRegistrations users={users} loading={loading} fetchStats={fetchStats} />;
      case 'settings':
    ...
    <div className="admin-dashboard">
      <div className="container">
        ...
        <div className="sidebar">
          ...
          <div
            className={`menu-item ${activeTab === 'registrations' ? 'active' : ''}`}
...
// Dashboard Overview Component - Fixed with Real Data
const DashboardOverview = ({ stats, onNavigate }) => (
  <div className="dashboard-overview">
    {/* Welcome + Stats Box - Exact Design Match */}
...
const EditEventModal = ({ event, onClose, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Parse time into startTime and endTime
      ...
      await axios.put(`/api/events/${event._id}`, eventData);
...
// Manage Events Component - Exact Design Match with Working Actions
const ManageEvents = ({ events, loading, onNavigate }) => {
  ...
  const handleDelete = async (eventId, eventTitle) => {
    if (window.confirm(`Are you sure you want to delete "${eventTitle}"? This action cannot be undone.`)) {
      try {
        ...
        // Refresh the events list without page reload
  ...
  const handleView = (event) => {
    ...
    toast.info(
      ...
      `Date: ${eventDate}\n` +
      `Location: ${event.location}\n` +
      `Registrations: ${registrations}/${event.maxParticipants}`,
      { duration: 5000 }
    );
  };
              ...
              {events.length > 0 ? events.map(event => (
                <tr key={event._id}>
                  ...
                  <td className="action-icons">
                    ...
                    <span className="edit" onClick={() => handleEdit(event)}>Edit</span>
...
// Create Event Component - Exact Design Match
const CreateEvent = () => {
  const [formData, setFormData] = useState({
    title: '',
    description: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Parse time into startTime and endTime for backend
      let startTime, endTime;

      if (formData.time.includes('–') || formData.time.includes('-')) {
        // Parse time range like "10:00 AM – 2:00 PM"
        const timeRange = formData.time.split(/[–-]/).map(t => t.trim());
        startTime = convertTo24Hour(timeRange[0]);
        endTime = convertTo24Hour(timeRange[1] || timeRange[0]);
      } else {
  ...
  const convertTo24Hour = (time12h) => {
    ...
    if (/^\d{1,2}:\d{2}$/.test(time12h.trim())) {
      return time12h.trim();
    }
...
// Student Registrations Component - Exact Design Match
const StudentRegistrations = ({ users, loading, fetchStats }) => {
  ...
  const handleRemove = async (userId, userName) => {
    if (window.confirm(`Are you sure you want to remove ${userName} from all events?`)) {
      try {
        ...
        await axios.delete(`/api/users/${userId}/registrations`);
  ...
  const handleSelect = async (userId, eventId, userName) => {
    try {
      await axios.post(`/api/events/admin/select-student`, {
        userId,
        eventId
      });
...
// Admin Settings Component - Exact Design Match
